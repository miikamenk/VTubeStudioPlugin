...
class Backend(BackendBase):
    ...
    def connect_auth(self):
        return self.loop.run_until_complete(self.vts.connect_auth())
        
    def getHotkeys(self)->list[str]:
        return self.loop.run_until_complete(self.vts.getHotkeys())

    def triggerHotkey(self, hotkey: str)->bool:
        return self.loop.run_until_complete(self.vts.triggerHotkey(hotkey))

    def moveModel(self, x: float, y: float, rot: float, size: float, relative: bool, move_time: float)->bool:
        return self.loop.run_until_complete(
            self.vts.moveModel(x, y, rot, size, relative, move_time)
        )
